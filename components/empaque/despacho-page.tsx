...
export function DespachoPage() {
  ...
  const exportToExcel = () => {
    const worksheetData = [
      ["Fecha", "Guía", "Cliente", "Transportista", "Pallets", "Destino", "Entrega Estimada", "Estado", "Responsable"],
      ...filteredDespachos.map((despacho) => [
        despacho.fecha,
        despacho.numeroGuia,
        despacho.cliente,
        despacho.transportista,
        despacho.pallets?.join("; ") || "Sin pallets",
        despacho.destino,
    ...
    XLSX.writeFile(workbook, `despachos-${today}.xlsx`)
  }
  ...
  const totalPallets = despachos.reduce((sum, d) => sum + (d.pallets?.length || 0), 0)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <Button variant="outline" onClick={exportToExcel} disabled={filteredDespachos.length === 0}>
                    ...
                    <TableRow key={despacho.id}>
                      ...
                      <TableCell>
                        <div className="text-sm">
                          <span className="font-medium">{despacho.pallets?.length || 0}</span>
                          <div className="text-xs text-muted-foreground">
                            {despacho.pallets?.slice(0, 2).map(p => p.split('-').pop()).join(', ') || 'Sin pallets'}
                            {(despacho.pallets?.length || 0) > 2 && '...'}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
